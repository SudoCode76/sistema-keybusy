...
begin
  ...
  select s.id, s.service_account_id, s.product_id, s.status, s.slot_label,
  ...
  if not found then
    raise exception 'Venta Spotify no encontrada';
  end if;
  ...
  select d.login_email, d.login_password, d.email_password, d.invitation_email
  into access_row
  ...
  -- Lock source and target accounts in deterministic order to avoid races.
  perform 1
  ...
  -- The subscription update trigger releases the old assignment. Keep the
  -- explicit update here as a defense against stale legacy assignments.
  ...
  if found then
    if destination_member.status = 'removed' then
      raise exception 'El miembro destino fue eliminado de Spotify y no puede reasignarse';
    end if;
    if destination_member.current_subscription_id is not null
    ...
    set login_password = access_row.login_password,
        ...
        invitation_email = access_row.invitation_email,
    ...
    insert into public.spotify_member_accounts(
      ...
      p_target_service_account_id,
      null,
      p_subscription_id,
      btrim(access_row.login_email),
      access_row.login_password,
      access_row.email_password,
      access_row.invitation_email,
...
revoke all on function public.move_spotify_member(uuid, uuid, uuid, uuid)
from public, anon;
grant execute on function public.move_spotify_member(uuid, uuid, uuid, uuid)
to authenticated;
