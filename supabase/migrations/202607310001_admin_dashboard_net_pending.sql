...
monthly_costs as (
  select
    coalesce(sum(c.amount_bob), 0)::numeric(12, 2) as cost_bob,
    coalesce(sum(c.amount_usdt), 0)::numeric(12, 2) as cost_usdt
  from public.costs c
  cross join bounds b
  where (c.paid_at at time zone 'America/La_Paz')::date between b.month_start and b.today
),
...
upcoming as (
  select
    coalesce(sum(r.amount_bob), 0)::numeric(12, 2) as upcoming_bob,
    coalesce(sum(r.amount_usdt), 0)::numeric(12, 2) as upcoming_usdt,
    count(*)::integer as upcoming_renewal_count
  from renewal_candidates r
  cross join bounds b
...
select
  ...
  mc.cost_bob,
  mc.cost_usdt,
  (c.collected_bob - mc.cost_bob)::numeric(12, 2) as net_collected_bob,
  (c.collected_usdt - mc.cost_usdt)::numeric(12, 2) as net_collected_usdt,
  u.upcoming_bob,
  u.upcoming_usdt,
  u.upcoming_renewal_count,
  (c.collected_bob - mc.cost_bob + p.pending_bob)::numeric(12, 2) as net_projected_bob,
  (c.collected_usdt - mc.cost_usdt + p.pending_usdt)::numeric(12, 2) as net_projected_usdt
from bounds b
